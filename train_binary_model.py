...
from untils import cross_entropy_loss, BinaryClassificationMetrics, plt_loss, plt_single

Image.MAX_IMAGE_PIXELS = 500000000


def train_binary_model(net, optimizer, scheduler, train_data, val_data, folder_dir, config, device=None):
    ...
    for epoch in range(config.epochs):
        ...
        for i, data in train_loop:
            ...
            outputs = net(inputs)
            loss = train_criterion(outputs, labels.to(device))
    ...
    metrics = BinaryClassificationMetrics(labels_all, predicted_labels_all)
    metrics.save_metrics("{}/binary_classification_metrics_{}.txt".format(folder_dir, config.net))
    metrics.plot_roc_curve(config, save_path="{}/roc_curve_{}.png".format(folder_dir, config.net))
